...
Account.prototype.merge = function (address, diff, cb) {
  ...
  this.editable.forEach(function (value) {
    if (diff[value]) {
      ...
      switch (self.conv[value]) {
        case String:
          if (value !== "balance" && value !== "u_balance") {
  ...
  var specialClumn = ['balance', 'u_balance']
  for (var i = 0; i < specialClumn.length; ++i) {
    var key = specialClumn[i]
    if (diff[key]) {
      var query
      if (diff[key][0] == '-') {
        query = 'update mem_accounts set ' + key + ' = ' + key + diff[key] + ' where address = "' + address + '"'
      } else {
        query = 'update mem_accounts set ' + key + ' = ' + key + ' + ' + diff[key] + ' where address = "' + address + '"'
      }
      sqles.push({

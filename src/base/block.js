...
Block.prototype.dbSave = function (block, cb) {
  ...
  this.scope.dbLite.query("INSERT INTO blocks(id, version, timestamp, height, previousBlock,  numberOfTransactions, totalFee, reward, payloadLength, payloadHash, generatorPublicKey, blockSignature) VALUES($id, $version, $timestamp, $height, $previousBlock, $numberOfTransactions, $totalFee, $reward, $payloadLength,  $payloadHash, $generatorPublicKey, $blockSignature)", {
    id: block.id,
...
Block.prototype.objectNormalize = function (block) {
  ...
  var report = this.scope.scheme.validate(block, {
    ...
    required: ['blockSignature', 'generatorPublicKey', 'numberOfTransactions', 'payloadHash', 'payloadLength', 'timestamp', 'totalFee', 'reward', 'transactions', 'version']
  });
...
Block.prototype.dbRead = function (raw) {
  if (!raw.b_id) {
    ...
    var block = {
      ...
      generatorId: addressHelper.generateBase58CheckAddress(raw.b_generatorPublicKey),

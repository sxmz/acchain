...
Transaction.prototype.verify = function (trs, sender, requester, cb) { //inheritance
  ...
  if (typeof trs.amount !== 'string') {
    return setImmediate(cb, "Invalid amount")
  }
  ...
  if (trs.type === TransactionTypes.SEND) {
    var error = amountHelper.validate(trs.amount)
    if (error) {
  ...
  try {
    ...
    library.logger.log('Got exception while verify asset body', e.toString())
...
Transaction.prototype.apply = function (trs, block, sender, cb) {
  ...
  var bNativeAmount = bignum(trs.fee)
  if (!trs.currency) {
    bNativeAmount = bNativeAmount.plus(trs.amount)
  }

  if (trs.blockId != genesisblock.block.id && bNativeAmount.gt(sender.balance)) {
    return setImmediate(cb, "Insufficient balance: " + sender.balance);
  }

  var self = this
  this.scope.account.merge(sender.address, {
    balance: '-' + bNativeAmount.toString(),
    blockId: block.id,
    ...
    if (trs.currency) {
      library.balanceCache.addAssetBalance(trs.recipientId, trs.currency, trs.amount)
      async.series([
        function (next) {
          library.model.updateAssetBalance(trs.currency, '-' + trs.amount, sender.address, next)
        },
        function (next) {
          library.model.updateAssetBalance(trs.currency, trs.amount, trs.recipientId, next)
        }
      ], function (err) {
        if (err) return cb(err)

        private.types[trs.type].apply.call(self, trs, block, sender, cb);
      })
    } else {
      private.types[trs.type].apply.call(self, trs, block, sender, cb);
    }
  ...
  this.scope.account.merge(sender.address, {
    ...
    if (trs.currency) {
      ...
      async.series([
        ...
        if (err) return cb(err)
        private.types[trs.type].undo.call(self, trs, block, sender, cb);
      })
    } else {
      private.types[trs.type].undo.call(self, trs, block, sender, cb);
    }
...
Transaction.prototype.applyUnconfirmed = function (trs, sender, requester, cb) {
  ...
  if (!trs.currency) {
    ...
    library.balanceCache.setAssetBalance(sender.address, trs.currency, surplus.toString())
  }
  ...
  var nativeAmount = '-' + bNativeAmount.toString()
  this.scope.account.merge(sender.address, { u_balance: nativeAmount }, function (err, sender) {
    if (err) return cb(err);
...
Transaction.prototype.undoUnconfirmed = function (trs, sender, cb) {
  ...
  if (!trs.currency) {
    ...
    library.balanceCache.addAssetBalance(sender.address, trs.currency, trs.amount)
  }
  var nativeAmount = bNativeAmount.toString()
  this.scope.account.merge(sender.address, { u_balance: nativeAmount }, function (err, sender) {
    if (err) return cb(err);

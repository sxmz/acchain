...
Round.prototype.getVotes = function (round, cb) {
  library.dbLite.query("select delegate, amount from ( " +
    ...
    ") where round = $round", { round: round }, { delegate: String, amount: String }, function (err, rows) {
      cb(err, rows)
    });
}
      ...
      function (cb) {
        self.getVotes(round, function (err, votes) {
          ...
          async.eachSeries(votes, function (vote, cb) {
            ...
            library.dbLite.query('update mem_accounts set vote = vote + ' + vote.amount + ' where address = $address', {
              address: address
            }, cb);
          }, function (err) {
            self.flush(round, function (err2) {
  ...
  modules.accounts.mergeAccountAndGet({
    ...
    async.series([
      ...
      function (cb) {
        var roundRewards = private.rewardsByRound[round]
        var totalRewards = 0
        for (var i = 0; i < roundRewards.length; ++i) {
          totalRewards += roundRewards[i]
        }
        library.logger.log('Round ' + round + ' produce ' + totalRewards + ' rewards')

        library.model.getRewardPoolBalance(function (err, rewardPoolBalance) {
          if (err) return cb(err)
          if (bignum(rewardPoolBalance).lt(totalRewards)) return cb('Insufficient reward balance')
          library.model.updateRewardPoolBalance('-' + totalRewards, cb)
        })
      }
    ], function (err) {
...
Round.prototype.onBlockchainReady = function () {
  ...
  library.dbLite.query("select sum(b.totalFee), GROUP_CONCAT(b.reward), GROUP_CONCAT(lower(hex(b.generatorPublicKey))) from blocks b where (select (cast(b.height / 101 as integer) + (case when b.height % 101 > 0 then 1 else 0 end))) = $round",
    ...
    },
    {
      ...
      for (var i = 0; i < rows[0].rewards.length; ++i) {
        rows[0].rewards[i] = parseInt(rows[0].rewards[i])
      }

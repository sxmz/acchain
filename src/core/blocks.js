...
private.blocksDataFields = {
  ...
  't_signatures': String,
  'approvals_topic': Number,
  'approvals_value': String,
  'issuers_name': String,
  'issuers_desc': String,
  ...
  'assets_estimateUnit': String,
  'assets_estimatePrice': String,
  'assets_exerciseUnit': String,
  'assets_extra': String,
...
private.list = function (filter, cb) {
  var sortFields = ['b.id', 'b.timestamp', 'b.height', 'b.previousBlock', 'b.totalFee', 'b.reward', 'b.numberOfTransactions', 'b.generatorPublicKey'];
  ...
  library.dbLite.query("select count(b.id) " +
    ...
    (fields.length ? "where " + fields.join(' and ') : ''), params, { count: Number }, function (err, rows) {
      ...
      library.dbLite.query("select b.id, b.version, b.timestamp, b.height, b.previousBlock, b.numberOfTransactions, b.totalFee, b.reward, b.payloadLength, lower(hex(b.payloadHash)), lower(hex(b.generatorPublicKey)), lower(hex(b.blockSignature)), (select max(height) + 1 from blocks) - b.height " +
        "from blocks b " +
        (fields.length ? "where " + fields.join(' and ') : '') + " " +
        (filter.orderBy ? 'order by ' + sortBy + ' ' + sortMethod : '') + " limit $limit offset $offset ", params, ['b_id', 'b_version', 'b_timestamp', 'b_height', 'b_previousBlock', 'b_numberOfTransactions', 'b_totalFee', 'b_reward', 'b_payloadLength', 'b_payloadHash', 'b_generatorPublicKey', 'b_blockSignature', 'b_confirmations'], function (err, rows) {
          if (err) {
...
private.getByField = function (field, cb) {
  ...
  library.dbLite.query("select b.id, b.version, b.timestamp, b.height, b.previousBlock, b.numberOfTransactions, b.totalFee, b.reward, b.payloadLength,  lower(hex(b.payloadHash)), lower(hex(b.generatorPublicKey)), lower(hex(b.blockSignature)), (select max(height) + 1 from blocks) - b.height " +
    "from blocks b " +
    "where " + condition, values, ['b_id', 'b_version', 'b_timestamp', 'b_height', 'b_previousBlock', 'b_numberOfTransactions', 'b_totalFee', 'b_reward', 'b_payloadLength', 'b_payloadHash', 'b_generatorPublicKey', 'b_blockSignature', 'b_confirmations'], function (err, rows) {
      if (err || !rows.length) {
...
shared.getSupply = function (req, cb) {
  ...
  library.model.getTotalSupply(function (err, totalSupply) {
    if (err) return cb('Failed to get supply')
    cb(null, { supply: totalSupply })
  })
}
...
shared.getStatus = function (req, cb) {
  ...
  var height = private.lastBlock.height

  library.model.getTotalSupply(function (err, totalSupply) {
    if (err) return cb('Failed to get supply')
    library.model.getRewardPoolBalance(function (err, rewardPoolBalance) {
      if (err) return cb('Failed to get reward poll')
      cb(null, {
        height: height,

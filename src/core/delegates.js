...
private.cacheActiveDelegates = null
...
Delegates.prototype.generateDelegateList = function (height, cb) {
  if (private.cacheActiveDelegates && private.cacheActiveDelegates.height === height) {
    return cb(null, private.cacheActiveDelegates.publicKeys)
  }
  private.getKeysSortByVote(function (err, truncDelegateList) {
    ...
    var addresses = truncDelegateList.map(function (publicKey) {
      return modules.accounts.generateAddressByPublicKey2(publicKey)
    })
    private.cacheActiveDelegates = {
      height: height,
      publicKeys: truncDelegateList,
      addresses: addresses
    }
...
Delegates.prototype.isActiveDelegate = function (address) {
  return private.cacheActiveDelegates.addresses.indexOf(address) != -1
}
...
Delegates.prototype.getDelegates = function (query, cb) {
  ...
  modules.accounts.getAccounts({
    ...
    library.model.getTotalSupply(function (err, totalSupply) {
      if (err) cb('Failed to get total supply')

...
function registerAsset(options) {
  if (!options.metafile || !fs.existsSync(options.metafile)) {
    console.error("Error: invalid params, asset meta file must exists");
    return;
  }
  var payload = JSON.parse(fs.readFileSync(options.metafile, 'utf8'));
  var trs = aschJS.uia.createAsset(payload, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}

function registerIssuer(options) {
  var trs = aschJS.uia.createIssuer(options.name, options.desc, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
function submitApproval(options) {
  var trs = aschJS.uia.createApproval(Number(options.topic), options.value, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}

function issueAsset(options) {
  var trs = aschJS.uia.createIssue(options.currency, options.amount, options.rate, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
module.exports = function (program) {
  ...
  program
    ...
    .description("register an issuer")
    .option("-e, --secret <secret>", "")
    .option("-s, --secondSecret <secret>", "")
    .option("-n, --name <issuer name>", "")
  ...
  program
    ...
    .option("-e, --secret <secret>", "")
    .option("-s, --secondSecret <secret>", "")
    .option("-f, --metafile <metafile", "asset meta file")
  ...
  program
    ...
    .option("-e, --secret <secret>", "")
    .option("-s, --secondSecret <secret>", "")
    .option("-t, --topic <topic>", "1: asset register, 2: asset issue")
    .option("-v, --value <value>", "currency for asset register, issueId for asset issue")
